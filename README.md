...
Claude Opus 4.8) compressed to ~13 minutes. It plays beside the slides for the rest
...
- `?profile=<name>` loads (and persists) a named preset from `SKIP_PROFILES` in
  `index.html` — ships with `full`, `tight15` (trims Act II/III detail),
  `results` (skips framework internals). Add your own per-venue sets there.
...
```bash
python3 prep_cast.py ../rbrun3.cast 800          # target 800 s of playback
...
- **Rest-point pacing**: `python3 prep_cast.py --fast-analyze` replays the cast
  ...
  the screen stayed still ≥60 s of original time (43 such story beats — almost
  always "an agent finished something and paused", matching the independent
  pyte ground-truth analysis) for 1.5–3 s, and compresses the motion in between
  uniformly to fill the target. The beats are drawn as amber ticks on the
...
The Zürich-temperature footer (open-meteo) and the EB Garamond webfont are
online-only niceties and degrade silently when offline.
